...
// 🚀 ONLY import the classes. Do not import 'connect' or 'isConnected' as functions.
...
export default function Home() {
  // 1. Create a stable session instance
  ...
  // 2. CHECK LOGIN ON MOUNT (The "Refresh" Recovery)
  useEffect(() => {
    setMounted(true);
  ...
  // 3. THE INSTANT CONNECT HANDLER (Fixes the refresh issue)
  const handleConnect = useCallback(() => {
    showConnect({
      ...
      userSession,
      onFinish: () => {
        // 🚀 THE MAGIC: Instead of reloading, we manually pull the data
        const userData = userSession.loadUserData();
        const address = userData.profile.stxAddress.mainnet || userData.profile.stxAddress.testnet;
        
        if (address) {
          setUserAddress(address); // Updates UI instantly
          setView("personal");     // Switches view instantly
          setShowToast(true);
  ...
  const handleLogout = useCallback(() => {
    ...
    setUserAddress(null);
    setView("global");
    // We only refresh on logout to clear provider caches
    window.location.reload(); 
  }, [userSession]);

  // --- DATA FETCHING & WEBSOCKET (Same as your working version) ---
      ...
      <AnimatePresence>
        {showToast && (
          <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} className="fixed top-20 right-6 z-[100] bg-emerald-600 text-white px-5 py-3 rounded-xl shadow-2xl flex items-center gap-3">
            <CheckCircle2 className="w-5 h-5" /> <span className="font-bold text-sm tracking-wide">Sync Successful</span>
          </motion.div>
        )}
      </AnimatePresence>
          ...
          {userAddress ? (
            <div className="flex items-center gap-2 bg-slate-900/80 p-1.5 rounded-2xl border border-white/5 shadow-2xl">
              <div className="flex bg-slate-800 rounded-xl p-1 items-center">
                ...
                <button onClick={() => setView("global")} className={`px-4 py-1.5 rounded-lg text-xs font-bold transition ${view === 'global' ? 'bg-orange-600 text-white shadow-lg' : 'text-slate-400 hover:text-white'}`}>Global</button>
                <button onClick={() => setView("personal")} className={`px-4 py-1.5 rounded-lg text-xs font-bold transition ${view === 'personal' ? 'bg-blue-600 text-white shadow-lg' : 'text-slate-400 hover:text-white'}`}>My Tx</button>
              </div>
            ...
            <button 
              ...
              className="flex items-center gap-2 px-8 py-3 bg-gradient-to-tr from-orange-600 to-orange-700 text-white rounded-xl font-bold transition-all hover:scale-[1.02] active:scale-95 shadow-xl shadow-orange-500/20"
            >
      ...
      <section className="space-y-4">
        <div className="flex items-center justify-between px-2">
          ...
          <span className="text-xs text-slate-500 font-mono">
            {userAddress ? "Connected" : "Monitoring Network..."}
          </span>
        </div>
